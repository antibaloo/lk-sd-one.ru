...
$(document).ready(function () {
    ...
    function analiticLoad(from, till,timezone) {
        var datalensDashboard = "https://datalens.yandex/66ffiy5ii5omt?_embedded=1&_no_controls=1";
        console.log("source from",from);
        from_shifted = moment.tz(from,timezone).tz("Europe/Moscow").format("YYYY-MM-DDTHH:mm")
        console.log("from with tz",from_shifted);
        console.log("source till",till);
        till_shifted = moment.tz(till,timezone).tz("Europe/Moscow").format("YYYY-MM-DDTHH:mm")
        console.log("till with tz",till_shifted);
